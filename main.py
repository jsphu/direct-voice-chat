...
def main():
    ...
    parser.add_argument("--mode", choices=["host", "join"], required=True, help="Run as host or join a session")
    parser.add_argument("--ip", help="Host IP address to connect to (required for join mode)")
    ...
    # Start threads
    send_thread = threading.Thread(target=capture_and_send, args=(audio_manager, network_manager, stop_event), daemon=True)
    recv_thread = threading.Thread(target=receive_and_play, args=(audio_manager, network_manager, stop_event), daemon=True)
